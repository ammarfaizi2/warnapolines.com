...
(function (root, factory) {
	...
	var supports = !!document.querySelector && !!root.addEventListener, // feature test
		...
		duDialog = function () {
			...
			var _ = this,
				_args = arguments,
				titleType = typeof _args[0],
				msgType = typeof _args[1],
						...
						if (e.target.matches('.dlg-action')) {
							...
							if (e.target.matches('.ok-action')) {
								if (_.config.selection && _.config.multiple) {
									var checked = content.querySelectorAll(':scope .dlg-select-checkbox:checked'),
										checkedVals = [],
										checkedItems = [];
					...
					if (e.type === 'keyup') {
						if (e.target.matches('.dlg-search')) {
							var _keyword = e.target.value,
								_items = content.querySelectorAll(':scope .dlg-select-item');
